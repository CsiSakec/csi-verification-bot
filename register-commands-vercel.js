// Register commands for Vercel-only webhook approach
...
const commands = [
  {
    ...
    // Don't set dm_permission: false to allow DMs, but handle context in response
  },
  {
    name: 'verifycode', 
...
(async () => {
  try {
    console.log('Started refreshing application (/) commands for Vercel-only approach...');
    ...
    console.log('✅ Successfully reloaded application (/) commands for Vercel-only!');
    console.log('📝 All commands now work with webhook-only approach');
    console.log('🌐 Bot will work in both DMs and servers via Vercel endpoint');
  } catch (error) {
